...
<body class="animsition site-menubar-disable-hover">
  ...
  <div class="page-header">
    <h1 class="page-title">Lista de Panaderia Registrada en DaAnJu</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a>Inicio</a></li>
      <li class="breadcrumb-item"><a>Gestion de Panaderia</a></li>
  </ol>
</div>
    ...
    <div class="panel-body">
                ...
                <table>
                    <tr>
                        <td>
                            @if (Auth::user()->tieneRole('Super Administrador'))
                            <a class="btn btn-primary" href="{{ route('panaderia.create') }}">Agregar Panaderia</a>
                            @endif
                              </td>
                    </tr>
                </table>
                ...
                <table class="table-hover dataTable table-striped w-full" id="exampleTableTools">

                    <thead>

                        <tr>
                            <th class="cell-300" scope="col">Id</th>
                            <th class="cell-300" scope="col">Nit</th>
                            <th class="cell-300" scope="col">Nombre</th>
                            <th class="cell-300" scope="col">Télefono</th>
                            <th class="cell-300" scope="col">Dirección</th>
                            <th class="cell-300" scope="col">Opciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($panaderia as $p)
                        <tr>
                            <td>{{ $p->id }}</td>
                            <td>{{ $p->nit }}</td>
                            <td>{{ $p->nombre }}</td>
                            ...
                            <td>
                                @if (Auth::user()->tieneRole('Super Administrador'))
                                <form method="get" action="{{ route('panaderia.edit',['panaderium'=>$p->id]) }}">
                                    <button type="submit" class="btn btn-info" name="editar">Editar</button>
                                </form>
                                <form method="post" action="{{ route('panaderia.destroy',['panaderium'=>$p->id]) }}">
                                    @csrf
                                    {{ method_field('delete') }}
                                    <button class="btn btn-danger" type="submit" onclick="return confirm('¿Desea eliminar esta Panaderia?');">Eliminar</button>
                                </form>
                                @endif
                            </td>
                        </tr>
